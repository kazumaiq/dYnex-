...
export const StickyReleaseStack: React.FC = () => {
  ...
  return (
    <section
      id="stack"
      className="relative w-full px-4 sm:px-8 lg:px-12 py-24 sm:py-32 pointer-events-none select-none max-w-7xl mx-auto"
    >
      <div className="w-full pointer-events-auto mb-10 sm:mb-14">
        <div className="flex items-center space-x-2 text-[10px] sm:text-xs font-mono text-signal-red uppercase tracking-widest-tech mb-2">
          <Layers size={13} className="text-cyber-purple-glow" />
      ...
      <div className="relative w-full space-y-8 sm:space-y-12 pb-16">
        {displayReleases.map((release, index) => {
          return (
            ...
            >
              <div className="tactical-border group relative w-full p-6 sm:p-10 bg-void-950/95 backdrop-blur-2xl border border-void-800 hover:border-signal-red/80 rounded-sm shadow-[0_20px_50px_rgba(0,0,0,0.85)] transition-all duration-300 overflow-hidden">
                <div
                  aria-hidden="true"
                  className="absolute right-4 bottom-2 font-sans font-black text-6xl sm:text-8xl lg:text-[9rem] tracking-tighter text-stroke-ghost select-none pointer-events-none opacity-25 leading-none"
                >
                  #{String(index + 1).padStart(2, '0')}
                </div>

                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="w-full md:w-72 sm:w-80 shrink-0 aspect-square rounded-sm overflow-hidden border border-void-700 group-hover:border-signal-red transition-colors relative shadow-2xl">
                    <img
                      src={release.artworkUrl}
                      alt={release.title}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                    />
                    <div className="absolute top-3 left-3 px-2 py-0.5 bg-void-950/90 backdrop-blur-sm border border-void-800 text-[10px] font-mono text-signal-red rounded-xs font-bold">
                      FLAGSHIP // {release.year}
                    </div>
                  </div>

                  <div className="flex-1 w-full font-mono space-y-3">
                    <div className="flex items-center space-x-2 text-xs text-cyber-purple-glow">
                    ...
                    <div className="text-sm text-technical-silver font-bold">
                      {release.artists}
                    </div>
                    ...
                    <div className="flex items-center space-x-4 pt-2 text-xs text-technical-silver">
                      <span className="px-2.5 py-1 bg-void-900 border border-void-800 text-[10px] text-signal-red font-bold rounded-xs">
                        {release.genre}
                      </span>
                    ...
                    <div className="barcode-pattern w-48 h-2.5 opacity-60 pt-2" />
                  </div>
                  ...
                  <div className="w-full md:w-auto shrink-0 flex flex-col items-end justify-center pt-4 md:pt-0 border-t md:border-t-0 border-void-800/80">
                    <button
                      ...
                      className="w-full md:w-auto px-6 py-3.5 bg-gradient-to-r from-signal-red to-signal-red-glow hover:shadow-neon-mix text-white font-mono text-xs tracking-widest uppercase rounded-sm flex items-center justify-center space-x-2.5 transition-all duration-200 active:scale-95"
                    >
