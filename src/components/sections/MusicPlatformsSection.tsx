  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto w-full pointer-events-auto">
        ...
        {/* High-Tech Grid with Black-Red-Violet Styling */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {platforms.map((p, idx) => {
            const isRed = p.accentColor === 'red';
            return (
              <motion.a
                key={p.id}
                href={p.url}
                ...
                className="group relative p-6 bg-gradient-to-br from-void-900/90 via-void-950/95 to-void-900/80 backdrop-blur-xl border border-void-800 rounded-sm transition-all duration-300 flex flex-col justify-between h-56 overflow-hidden hover:border-cyber-purple/70 hover:shadow-neon-mix cursor-pointer"
              >
                {/* Subtle cyber background line accent */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-cyber-purple/10 to-transparent pointer-events-none rounded-bl-full" />
                <div className="absolute -bottom-10 -left-10 w-24 h-24 bg-gradient-to-tr from-signal-red/10 to-transparent pointer-events-none rounded-tr-full" />
                ...
                <div className="flex items-start justify-between relative z-10">
                  <div className="flex items-center space-x-3">
                    ...
                    <div
                      className={`w-11 h-11 rounded-sm bg-void-950 border transition-all duration-300 flex items-center justify-center ${
                        isRed
                          ? 'border-void-800 text-signal-red group-hover:border-signal-red group-hover:shadow-[0_0_12px_rgba(230,25,36,0.35)]'
                          : 'border-void-800 text-cyber-purple group-hover:border-cyber-purple group-hover:shadow-[0_0_12px_rgba(139,92,246,0.35)]'
                      }`}
                    >
                      {p.svgIcon}
                    </div>
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-mono tracking-wider text-technical-muted group-hover:text-cyber-purple-glow transition-colors">
                        {p.protocolTag}
                      </span>
                      <div className="flex items-center space-x-1 mt-0.5">
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${
                            isRed ? 'bg-signal-red' : 'bg-cyber-purple'
                          }`}
                        />
                  ...
                  <div className="w-8 h-8 rounded-sm bg-void-950/80 border border-void-800/80 flex items-center justify-center text-technical-muted group-hover:text-white group-hover:border-cyber-purple/60 group-hover:bg-void-900 transition-all">
                    <ArrowUpRight
                      size={15}
                      className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-cyber-purple-glow transition-all duration-200"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-end space-x-1 my-2 py-1 opacity-40 group-hover:opacity-100 transition-opacity">
                  <span className="w-1 h-2 group-hover:h-4 bg-signal-red rounded-full transition-all duration-300" />
                  <span className="w-1 h-4 group-hover:h-2 bg-cyber-purple rounded-full transition-all duration-200" />
                  <span className="w-1 h-3 group-hover:h-5 bg-signal-red-glow rounded-full transition-all duration-400" />
                  <span className="w-1 h-5 group-hover:h-3 bg-cyber-purple-glow rounded-full transition-all duration-250" />
                  <span className="w-1 h-2 group-hover:h-4 bg-white/70 rounded-full transition-all duration-350" />
                </div>

                {/* Bottom Row: Platform Name + Description + Stream CTA */}
                <div className="relative z-10 border-t border-void-800/60 pt-3">
                  <div className="flex items-center justify-between">
                    <div className="text-lg font-bold font-sans text-white group-hover:text-cyber-purple-glow transition-colors">
                      {p.name}
                    </div>
        ...
        <div className="mt-8 p-4 bg-void-900/60 backdrop-blur-md border border-void-800/80 rounded-sm flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-mono text-technical-muted">
          ...
          <div className="text-[11px] text-cyber-purple-glow">
